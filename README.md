...
- Window / table calculations (`RUNNING_SUM`, `RUNNING_AVG/MIN/MAX`, `WINDOW_SUM/AVG/MIN/MAX/COUNT`, `LOOKUP`, `RANK`, `RANK_DENSE`, `RANK_UNIQUE`, `INDEX`, `FIRST`, `LAST`) → A `kind:sql` helper element with explicit Snowflake `OVER()` clauses (Sigma's DM formulas have no working partitioned/ordered window equivalents). Partition keys come from worksheet `rows` shelves; order keys from time-truncated `cols` shelves. Multiple window calcs sharing the same partition+order share a single helper.
- Table calcs the SQL lowering can't express → 🧩 **workbook patterns** (reported with ready-to-place formulas, never injected as DM columns — Sigma window functions silently error in data-model calc columns/metrics). Validated mappings: `RUNNING_*(agg)`→`Cumulative*(agg)`; `WINDOW_AVG/SUM/MAX/MIN(agg, -n, 0)`→`Moving*(agg, n)` (and `(-n, m)`→`(agg, n, m)`); `WINDOW_STDEV`→`MovingStdDev`; `agg/WINDOW_SUM(agg)`→`PercentOfTotal(agg, "grand_total")`; `RUNNING_SUM(agg)/TOTAL(agg)`→`CumulativeSum(PercentOfTotal(…))`; `RANK/RANK_DENSE/RANK_PERCENTILE`→`Rank/RankDense/RankPercentile(agg, "desc")`; `INDEX()`→`RowNumber()`; `LOOKUP(agg, ±n)`→`Lag/Lead(agg, n)`. Place each formula in a GROUPED workbook element (group by the chart's dimensions). The converter never emits `*Over` functions.
...
- Data blends (workbook-level `<datasource-relationships>` linking two+ sources on shared dimensions) → ONE merged data model. The secondary is pre-grouped to the link grain so the relationship is `many_to_one` — required because Sigma relationships are lookups: a row-grain many-to-many key returns `NULL` (the blended measure is silently lost), not a fan-out join. The looked-up secondary measure is surfaced with a non-additive aggregate (`Max`), since it is constant per link key (`Sum` would fan it out by primary-row count). Cross-source calcs like `SUM([Sales])-SUM([Target])` auto-translate to the blended grain; complex cross-source formulas are flagged. Query the `<PRIMARY>_BLENDED` element.
...
- Untranslatable table calcs — `WINDOW_MEDIAN`, `WINDOW_PERCENTILE`, `WINDOW_CORR`, `WINDOW_COVAR`, `PREVIOUS_VALUE` (self-referential — a LAG over the inner aggregate is NOT equivalent), and `SIZE()` have no Sigma equivalent. Each is flagged loudly with the untranslated fragment named (warning + 🧩 unsupported workbook-pattern entry) — never silently dropped, never faked. Table calcs embedded inside larger expressions are likewise flagged rather than emitted broken.
